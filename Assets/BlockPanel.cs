...
public class BlockPanel : MonoBehaviour
{
    ...
    IEnumerator IESlideBlock(int index, int targetIndex)
    {
        ...
        {
            yield return StartCoroutine(IERemoveBlocks(matchBlocks));
    ...
    IEnumerator IEMediate()
    {
        ...
        do
        {
            ...
            foreach (Block block in panelBlocks)
            {
                ...
                {
                    movingCount++;
                    block.transform.localPosition = Vector3.MoveTowards(block.transform.localPosition, destination, 5f * Time.deltaTime);
                }
            }
            yield return null;
    ...
    private IEnumerator IERemoveBlocks(Block[] removeBlocks)
    {
